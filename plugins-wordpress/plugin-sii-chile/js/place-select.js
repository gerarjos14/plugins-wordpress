(function ($) {
  ...
  /**
   * Obtenemos los campos del tipo, el rut y el gito para trabajar con ellos
   */
  var select_type_doc = document.getElementById('billing_type');

  var rut = document.getElementById('billing_rut_field');
  var rut_input = document.getElementById('billing_rut');
  var giro = document.getElementById('billing_giro_field');
  var giro_input = document.getElementById('billing_giro');

  /**
   * Ocultamos el giro y el rut de la vista por defecto
   */
  rut.style.display = 'none'
  giro.style.display = 'none'

  /**
   * seteamos estos datos por defecto si es boleta electronica para que pase la validacion
   */
  rut_input.value="66666666-6";
  giro_input.value="empty";  


  /**
   * escuchamos el evento de cambio del select
   */
  select_type_doc.addEventListener('change', (event) => {
    
    if (select_type_doc.value == 'invoice') {
      //Si es factura, mostramos los campos y los vaciamos
      ...
      //si es boleta, olcultamos los campos y agregamos los valores por defecto
      rut.style.display = 'none'
      giro.style.display = 'none'
      rut_input.value="66666666-6";
      giro_input.value="empty";      
