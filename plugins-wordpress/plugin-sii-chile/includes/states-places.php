...
// field p and label
    ...
    /**
     * Obtenemos los datos del script de Analitycs y los almacenamos en una tabla local.
     */
    function addCommittee() {
        global $wpdb;      
      
        if(isset($_POST['big_data'])&&isset($_POST['big_data']))
        {
            $big_data  = $_POST['big_data'];
            $web_url   = $_POST['web_url'];
            $id_unique = $_POST['id_usuario'];

            $wpdb->insert(BC_TABLE_ANALITYCS,
                array(
                'visitor_id' => get_current_user_id(),
                'big_data'   => $big_data,
                'web_url'    => $web_url,
                'id_usuario' => $id_unique,
    ...
    public function load_scripts()
    {
        ...
        $staristics = self::get_plugin_url() . 'js/user-statistics.js';

        wp_register_script( 'user-statistics', $staristics, array( 'jquery', 'woocommerce' ),self::VERSION, true );

        $JS_HELPERS = array(
            'host' => get_site_url(),
            'token'=> $row->token,
            'billconnecto_url'=>BC_URL_API,
            'user_logued'=>get_current_user_id(),
            'save_analitycs'=>$this->addCommittee(),
          );

        wp_localize_script( 'user-statistics', 'WP_OPTIONS', $JS_HELPERS );
        wp_enqueue_script('user-statistics');
