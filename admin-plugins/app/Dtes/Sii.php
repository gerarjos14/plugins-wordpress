...
class Sii
{
    ...
    public static function wsdl($servicio, $ambiente = null)
    {
        ...
        if ($ambiente==self::CERTIFICACION) {
            $wsdl = resource_path() .'/wsdl/'.self::$config['servidor'][$ambiente].'/'.$servicio.'.jws';
            if (is_readable($wsdl))
                return $wsdl;
        }
    ...
    public static function request($wsdl, $request, $args = null, $retry = null)
    {
        ...
        if (!self::$verificar_ssl) {
            if (self::getAmbiente()==self::PRODUCCION) {
                ...
                \App\Dtes\Log::write(Estado::ENVIO_SSL_SIN_VERIFICAR, $msg, LOG_WARNING);
            }
        ...
        try {
            ...
            \App\Dtes\Log::write(Estado::REQUEST_ERROR_SOAP, Estado::get(Estado::REQUEST_ERROR_SOAP, $msg));
            return false;
        }
        for ($i=0; $i<$retry; $i++) {
            try {
        ...
        if ($body===null) {
            \App\Dtes\Log::write(Estado::REQUEST_ERROR_BODY, Estado::get(Estado::REQUEST_ERROR_BODY, $wsdl, $retry));
            return false;
        }
    ...
    public static function enviar($usuario, $empresa, $dte, $token, $gzip = false, $retry = null)
    {
        ...
        if ($gzip) {
            $dte = gzencode($dte);
            if ($dte===false) {
                \App\Dtes\Log::write(Estado::ENVIO_ERROR_GZIP, Estado::get(Estado::ENVIO_ERROR_GZIP));
                return false;
            }
        }
        ...
        $header = [
            ...
            'Referer: https://www.e2fletcher.com',
            'Cookie: TOKEN='.$token,
        ];
        ...
        if (!$response or $response=='Error 500') {
            if (!$response) {
                \App\Dtes\Log::write(Estado::ENVIO_ERROR_CURL, Estado::get(Estado::ENVIO_ERROR_CURL, curl_error($curl)));
            }
            if ($response=='Error 500') {
                \App\Dtes\Log::write(Estado::ENVIO_ERROR_500, Estado::get(Estado::ENVIO_ERROR_500));
            }
            return false;
        }
        // cerrar sesión curl
        ...
        try {
            ...
            \App\Dtes\Log::write(Estado::ENVIO_ERROR_XML, Estado::get(Estado::ENVIO_ERROR_XML, $e->getMessage()));
            return false;
        }
        if ($xml->STATUS!=0) {
            \App\Dtes\Log::write(
    ...
    public static function cert($idk = null)
    {
        ...
        if ($idk) {
            $cert = resource_path() .'/certs/'.$idk.'.cer';
            if (is_readable($cert)) {
        ...
        $cert = resource_path() .'/certs/'.self::$config['certs'][$ambiente].'.cer';
        if (!is_readable($cert)) {
            \App\Dtes\Log::write(Estado::SII_ERROR_CERTIFICADO, Estado::get(Estado::SII_ERROR_CERTIFICADO, self::$config['certs'][$ambiente]));
            return false;
        }
        return file_get_contents($cert);
    }
    ...
    {
        if ($ambiente===null) {
            if (defined('_DTES_CERTIFICACION_'))
                $ambiente = (int)_DTES_CERTIFICACION_;
            else
