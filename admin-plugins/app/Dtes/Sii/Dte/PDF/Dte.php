...
namespace App\Dtes\Sii\Dte\PDF;

class Dte extends \App\Dtes\PDF
{

    use \App\Dtes\Sii\Dte\Base\DteImpreso;
    ...
    {
        parent::__construct();
        $this->SetTitle('Documento Tributario Electrónico (DTE) de Chile by DTES');
    ...
    protected function agregarEmisor(array $emisor, $x = 10, $y = 15, $w = 75, $w_img = 30, $font_size = null, array $color = null, $h_folio = null, $w_all = null)
    {
        ...
        if ($agregarDatosEmisor) {
            ...
            $ciudad = !empty($emisor['CiudadOrigen']) ? $emisor['CiudadOrigen'] : \App\Dtes\Chile::getCiudad($comuna);
    ...
    protected function agregarFolio($rut, $tipo, $folio, $sucursal_sii = null, $x = 130, $y = 15, $w = 70, $font_size = null, array $color = null)
    {
        ...
        if ($tipo) {
            $this->Texto('S.I.I. - '.\App\Dtes\Sii::getDireccionRegional($sucursal_sii), $x, $this->getY()+4, 'C', $w);
        }
        $this->SetTextColorArray([0,0,0]);
        $this->Ln();
        return $this->y;
    ...
    protected function agregarReceptor(array $Encabezado, $x = 10, $offset = 22)
    {
        ...
        if (!empty($receptor['DirRecep'])) {
            ...
            $ciudad = !empty($receptor['CiudadRecep']) ? $receptor['CiudadRecep'] : (
                !empty($receptor['CmnaRecep']) ? \App\Dtes\Chile::getCiudad($receptor['CmnaRecep']) : ''
            );
        ...
        if (!empty($receptor['Extranjero']['Nacionalidad'])) {
            ...
            $this->Texto(':', $x+$offset);
            $this->setFont('', '', null);
            $this->MultiTexto(\App\Dtes\Sii\Aduana::getNacionalidad($receptor['Extranjero']['Nacionalidad']), $x+$offset+2);
        }
    ...
    protected function agregarTraslado($IndTraslado, array $Transporte = null, $x = 10, $offset = 22)
    {
        ...
        if (!empty($Transporte['Aduana']) and is_array($Transporte['Aduana'])) {
            ...
            foreach ($Transporte['Aduana'] as $tag => $codigo) {
                ...
                $glosa = \App\Dtes\Sii\Aduana::getGlosa($tag);
                $valor = \App\Dtes\Sii\Aduana::getValor($tag, $codigo);
    ...
    protected function agregarTotales(array $totales, $otra_moneda, $y = 190, $x = 145, $offset = 25)
    {
        ...
        if (!empty($totales['ImptoReten'])) {
            ...
            foreach($ImptoReten as $i) {
                ...
                if (!empty($i['TasaImp'])) {
                    $glosas['ImptoReten_'.$i['TipoImp']] = \App\Dtes\Sii\ImpuestosAdicionales::getGlosa($i['TipoImp']).' ('.$i['TasaImp'].'%) $';
                } else {
                    $glosas['ImptoReten_'.$i['TipoImp']] = \App\Dtes\Sii\ImpuestosAdicionales::getGlosa($i['TipoImp']).' $';
                }
            }
