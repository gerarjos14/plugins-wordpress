...
class Estado
{
    ...
    // códigos de error para \App\Dtes\Sii::enviar()
    const ENVIO_OK = 0;
    ...
    const ENVIO_ERROR_GZIP = 55; /// código inventado en (no oficial SII)

    // códigos de error para \App\Dtes\Sii::request()
    const REQUEST_ERROR_SOAP = 61;
    const REQUEST_ERROR_BODY = 62;

    // códigos de error para \App\Dtes\Sii::cert()
    const SII_ERROR_CERTIFICADO = 71;

    // códigos de error para \App\Dtes\FirmaElectronica
    const FIRMA_ERROR = 101;

    // códigos de error para \App\Dtes\File::compress()
    const COMPRESS_ERROR_READ = 201;
    const COMPRESS_ERROR_ZIP = 202;

    // códigos de error para \App\Dtes\Sii\Autenticacion
    const AUTH_ERROR_SEMILLA = 301;
    const AUTH_ERROR_FIRMA_SOLICITUD_TOKEN = 302;
    const AUTH_ERROR_TOKEN = 303;

    // códigos de error para \App\Dtes\Sii\Dte
    ...
    // códigos de error para \App\Dtes\Sii\EnvioDte
    ...
    // códigos de error para \App\Dtes\Sii\EnvioRecibos
    const ENVIORECIBOS_FALTA_RECIBO = 601;
    const ENVIORECIBOS_FALTA_CARATULA = 602;

    // códigos de error para \App\Dtes\Sii\Folios
    ...
    // códigos de error para \App\Dtes\Sii\RespuestaEnvio
    const RESPUESTAENVIO_FALTA_RESPUESTA = 801;
    const RESPUESTAENVIO_FALTA_CARATULA = 802;

    // códigos de error para \App\Dtes\Sii\Base\Documento
    ...
    private static $glosas = [
        ...
        // códigos de error para \App\Dtes\Sii\LibroGuia
